...
class BasketballDetector:
    def __init__(self, show_inference=False):
        ...
        # 回调函数
        self.on_clip_saved = None  # 视频片段保存完成后的回调函数
    ...
    def save_clip(self, video_path, goal_frame, fps, output_dir):
        ...
        try:
            ...
            # 生成输出文件路径，使用帧号作为文件名
            output_path = os.path.join(output_dir, f'shot_{goal_frame:06d}.mp4')
            temp_path = output_path + "_temp.mp4"
            
            print(f"开始保存视频片段，输出路径: {output_path}")
            ...
            try:
                ...
                if result.returncode != 0:
                    ...
                    print(f"FFmpeg转换成功，视频已保存到: {output_path}")
                    # 调用回调函数
                    if self.on_clip_saved:
                        self.on_clip_saved(goal_frame, output_path)
        ...
        if os.path.exists(output_path):
            print(f"确认：视频片段已成功保存到 {output_path}")
            print(f"文件大小: {os.path.getsize(output_path)} 字节")
        else:
            print(f"错误：最终视频文件不存在 {output_path}")
    ...
    def detect_goals(self, video_path, output_dir='output_clips', progress_callback=None):
        """检测视频中的进球"""
        ...
        try:
            while cap.isOpened():
                ...
                if progress_callback:
                    progress_callback.update(frame_count, total_frames)
